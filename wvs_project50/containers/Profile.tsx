...
import { useUserApi } from "@/containers/hooks/useUserApi";
...
const Profile = () => {
    const { user, setUser, claims, loadingMessage, setLoadingMessage } = useSocial();
    const { GetUser, handleUpdataUser, handleUpdateCustomData } = useUserApi();
    ...
    const onSave = async () => {
        ...
        if(user?.avatar !== avatar){
            console.log("需要更新頭像");
            await handleUpdataUser(claims.sub, {
                "avatar": avatar
            }, setUser).catch(error => {
        ...
        if(user?.customData?.description !== description){
            ...
            await handleUpdateCustomData(claims.sub, {
    ...
    const onRefresh = useCallback(async() => {
        ...
        const newUser = await GetUser(claims?.sub);
        setUser(newUser);
    ...
    return (
        ...
        <ThemedScrollView
            contentContainerStyle={{ flexGrow: 1 }}  // 確保內容區域可以佔滿整個畫面
            refreshControl={
